...
def test_c12_chitiet_sm2_recipe_uses_updated_report_contract():
    recipe_path = (
...
def test_c12_chitiet_sm2_downloader_passes_month_and_center_overrides(monkeypatch):
    ...
    def fake_download_with_recipe(*args, **kwargs):
        ...
        return "/tmp/c1.2_chitiet_sm2_report.xlsx"

    monkeypatch.setattr(downloaders, "download_with_recipe", fake_download_with_recipe)

    result = downloaders.download_report_c12_chitiet_sm2_api(
        month_id="98944805",
    ...
    assert result == "/tmp/c1.2_chitiet_sm2_report.xlsx"
    assert captured["args"] == ("c12_chitiet_sm2_q2_2026",)
    ...
    assert captured["kwargs"]["output_name"] == "c1.2_chitiet_sm2_report.xlsx"


def test_c12_chitiet_sm2_batch_task_uses_month_center_id():
    from api_transition.batch_download import REPORT_TASKS

    task = next(task for task in REPORT_TASKS if task.report_key == "c12_chi_tiet_sm2")

    assert task.name == "C1.2 Chi tiết SM2"
