# -*- coding: utf-8 -*-
"""
Module xử lý giảm trừ phiếu báo hỏng theo phương pháp HNI (Hà Nội) - v2

Refactored:
- Import scoring từ kpi_scoring.py (single source of truth)
- Import team mapping từ team_config.py
- Import tính KPI từ kpi_calculator_v2.py
- Không import report_generator (tách biệt phần tạo Word)

CƠ CHẾ GIẢM TRỪ HNI:
- KHÁC với cách cũ: Không loại bỏ phiếu hoàn toàn khỏi mẫu số
- CHỈ chuyển trạng thái phiếu từ "KHÔNG ĐẠT" → "ĐẠT" trong tử số
- MẪU SỐ: Giữ nguyên = Tổng phiếu thô
- TỬ SỐ: = Số phiếu đạt gốc + Số phiếu GT (không đạt → đạt)
"""
...
import kpi_calculator_v2
from kpi_calculator_v2 import tinh_diem_kpi_nvkt, tinh_diem_kpi_nvkt_sau_giam_tru
from team_config import BRCD_TEAMS

# Build team short name mapping from team_config (single source of truth)
TEAM_SHORT_NAMES = {}
...
def add_bsc_scores_to_c12_tp2(file_path):
    ...
    try:
        ...
        if 'So_sanh_chi_tiet' in excel_file.sheet_names:
            ...
            df['Điểm BSC (Thô)'] = df['Tỷ lệ báo hỏng (%) (Thô)'].apply(lambda x: tinh_diem_C12_TP2(chuan_hoa_ty_le(x))).round(2)
            df['Điểm BSC (Sau GT)'] = df['Tỷ lệ báo hỏng (%) (Sau GT)'].apply(lambda x: tinh_diem_C12_TP2(chuan_hoa_ty_le(x))).round(2)
...
def add_bsc_scores_to_c15(file_path):
    ...
    try:
        ...
        if 'So_sanh_chi_tiet' in excel_file.sheet_names:
            ...
            # Tính điểm BSC
            df['Điểm BSC (Thô)'] = df['Tỷ lệ đạt (%) (Thô)'].apply(lambda x: tinh_diem_C15(chuan_hoa_ty_le(x))).round(2)
            df['Điểm BSC (Sau GT)'] = df['Tỷ lệ đạt (%) (Sau GT)'].apply(lambda x: tinh_diem_C15(chuan_hoa_ty_le(x))).round(2)
            df['Chênh lệch Điểm'] = (df['Điểm BSC (Sau GT)'] - df['Điểm BSC (Thô)']).round(2)
...
def add_bsc_scores_to_c14(file_path):
    ...
    try:
        ...
        if 'So_sanh_chi_tiet' in excel_file.sheet_names:
            ...
            df['Điểm BSC (Thô)'] = df['Tỷ lệ HL (%) (Thô)'].apply(lambda x: tinh_diem_C14(chuan_hoa_ty_le(x))).round(2)
            df['Điểm BSC (Sau GT)'] = df['Tỷ lệ HL (%) (Sau GT)'].apply(lambda x: tinh_diem_C14(chuan_hoa_ty_le(x))).round(2)
            df['Chênh lệch Điểm'] = (df['Điểm BSC (Sau GT)'] - df['Điểm BSC (Thô)']).round(2)
...
def add_bsc_scores_to_c12_sm1(file_path):
    ...
    try:
        ...
        if 'So_sanh_chi_tiet' in excel_file.sheet_names:
            ...
            df['Điểm BSC (Thô)'] = df['Tỷ lệ HLL % (Thô)'].apply(lambda x: tinh_diem_C12_TP1(chuan_hoa_ty_le(x))).round(2)
            df['Điểm BSC (Sau GT)'] = df['Tỷ lệ HLL % (Sau GT)'].apply(lambda x: tinh_diem_C12_TP1(chuan_hoa_ty_le(x))).round(2)
            df['Chênh lệch Điểm'] = (df['Điểm BSC (Sau GT)'] - df['Điểm BSC (Thô)']).round(2)
...
def add_bsc_scores_to_c11_sm4(file_path):
    ...
    try:
        ...
        if 'So_sanh_chi_tiet' in excel_file.sheet_names:
            ...
            # Tính điểm BSC
            df['Điểm BSC (Thô)'] = df['Tỷ lệ % (Thô)'].apply(lambda x: tinh_diem_C11_TP2(chuan_hoa_ty_le(x))).round(2)
            df['Điểm BSC (Sau GT)'] = df['Tỷ lệ % (Sau GT)'].apply(lambda x: tinh_diem_C11_TP2(chuan_hoa_ty_le(x))).round(2)
            df['Chênh lệch Điểm'] = (df['Điểm BSC (Sau GT)'] - df['Điểm BSC (Thô)']).round(2)
...
def add_bsc_scores_to_c11_sm2(file_path):
    ...
    try:
        ...
        if 'So_sanh_chi_tiet' in excel_file.sheet_names:
            ...
            # Tính điểm BSC
            df['Điểm BSC (Thô)'] = df['Tỷ lệ % (Thô)'].apply(lambda x: tinh_diem_C11_TP1(chuan_hoa_ty_le(x))).round(2)
            df['Điểm BSC (Sau GT)'] = df['Tỷ lệ % (Sau GT)'].apply(lambda x: tinh_diem_C11_TP1(chuan_hoa_ty_le(x))).round(2)
            df['Chênh lệch Điểm'] = (df['Điểm BSC (Sau GT)'] - df['Điểm BSC (Thô)']).round(2)
...
def calculate_statistics_hni(df_raw, exclusion_ids, has_ten_doi=True, 
                              dat_column='DAT_TT_KO_HEN', dat_value=1):
    """
    Tính toán thống kê theo cơ chế HNI (Hà Nội):
    - MẪU SỐ: Giữ nguyên = Tổng phiếu thô
    - TỬ SỐ: = Số phiếu đạt gốc + Số phiếu GT (không đạt → đạt)
    
    Args:
        df_raw: DataFrame dữ liệu thô (đã có cột NVKT và BAOHONG_ID_STR)
        exclusion_ids: Set các BAOHONG_ID cần giảm trừ
        has_ten_doi: Có cột TEN_DOI hay không
        dat_column: Tên cột để xác định phiếu đạt
        dat_value: Giá trị để xác định phiếu đạt
        
    Returns:
        DataFrame với thống kê theo cách HNI
    """
    report_data = []
    ...
    for group_key, group_df in df_raw.groupby(group_columns):
        # Tổng phiếu = Tổng phiếu thô (giữ nguyên mẫu số)
        tong_phieu = len(group_df)
        
        # Số phiếu đạt gốc
        so_phieu_dat_goc = len(group_df[group_df[dat_column] == dat_value])
        
        # Số phiếu trong danh sách GT mà KHÔNG ĐẠT (sẽ được chuyển thành ĐẠT)
        df_gt = group_df[group_df['BAOHONG_ID_STR'].isin(exclusion_ids)]
        so_phieu_gt_khong_dat = len(df_gt[df_gt[dat_column] != dat_value])
        
        # Số phiếu đạt SAU GT = Đạt gốc + GT không đạt (chuyển thành đạt)
        so_phieu_dat_sau_gt = so_phieu_dat_goc + so_phieu_gt_khong_dat
        
        # Tỷ lệ % với MẪU SỐ = Tổng phiếu thô (không đổi)
        ty_le = round((so_phieu_dat_sau_gt / tong_phieu * 100), 2) if tong_phieu > 0 else 0
        ...
        if has_ten_doi:
            ...
            report_data.append({
                'TEN_DOI': ten_doi,
                'NVKT': nvkt,
                'Tổng phiếu': tong_phieu,  # Giữ nguyên = tổng thô
                'Số phiếu đạt': so_phieu_dat_sau_gt,  # Đạt gốc + GT không đạt
                'Số phiếu GT': so_phieu_gt_khong_dat,  # Số phiếu được GT
                'Tỷ lệ %': ty_le
            })
        else:
            report_data.append({
                'NVKT': group_key,
                'Tổng phiếu': tong_phieu,
                'Số phiếu đạt': so_phieu_dat_sau_gt,
                'Số phiếu GT': so_phieu_gt_khong_dat,
                'Tỷ lệ %': ty_le
            })
...
def calculate_unit_stats_hni(df_stats_before, df_stats_after, ten_doi_col='TEN_DOI',
                              tong_col='Tổng phiếu', dat_col='Số phiếu đạt'):
    """
    Tính thống kê theo từng đơn vị (Tổ) theo cơ chế HNI
    
    LƯU Ý HNI:
    - Tổng phiếu (Sau GT) = Tổng phiếu (Thô) - không thay đổi
    - Tỷ lệ % (Sau GT) = Phiếu đạt (Sau GT) / Tổng phiếu (Thô) × 100
    
    Args:
        df_stats_before: DataFrame thống kê TRƯỚC giảm trừ
        df_stats_after: DataFrame thống kê SAU giảm trừ (theo HNI)
    ...
    if ten_doi_col in df_stats_before.columns:
        # Nhóm theo đơn vị TRƯỚC giảm trừ
        unit_before = df_stats_before.groupby(ten_doi_col).agg({
            tong_col: 'sum',
            dat_col: 'sum'
        }).reset_index()
        
        # Nhóm theo đơn vị SAU giảm trừ (HNI: tổng phiếu không đổi)
        agg_dict = {tong_col: 'sum', dat_col: 'sum'}
        if 'Số phiếu GT' in df_stats_after.columns:
            agg_dict['Số phiếu GT'] = 'sum'
        unit_after = df_stats_after.groupby(ten_doi_col).agg(agg_dict).reset_index()
        ...
        for _, row in unit_merged.iterrows():
            ...
            # HNI: Tổng phiếu sau GT = Tổng phiếu thô (không đổi)
            tong_sau = tong_tho  # Giữ nguyên mẫu số
            dat_tho = row.get(f'{dat_col} (Thô)', 0) or 0
            dat_sau = row.get(f'{dat_col} (Sau GT)', 0) or 0
            so_phieu_gt = row.get('Số phiếu GT', 0) or 0
            
            # Tỷ lệ với mẫu số = Tổng phiếu thô
            tyle_tho = round((dat_tho / tong_tho * 100), 2) if tong_tho > 0 else 0
            tyle_sau = round((dat_sau / tong_tho * 100), 2) if tong_tho > 0 else 0  # HNI: dùng tong_tho
            
            unit_stats.append({
                'Đơn vị': short_name,
                'Tổng phiếu (Thô)': int(tong_tho),
                'Phiếu GT (KĐ→Đ)': int(so_phieu_gt),  # Số phiếu chuyển từ không đạt → đạt
                'Tổng phiếu (Sau GT)': int(tong_sau),  # = Tổng thô
    ...
    # Thêm dòng TỔNG TTVT
    tong_tho_all = df_stats_before[tong_col].sum()
    dat_tho_all = df_stats_before[dat_col].sum()
    dat_sau_all = df_stats_after[dat_col].sum()
    so_phieu_gt_all = df_stats_after['Số phiếu GT'].sum() if 'Số phiếu GT' in df_stats_after.columns else 0
    
    # HNI: Tổng phiếu sau GT = Tổng phiếu thô
    tong_sau_all = tong_tho_all
    
    tyle_tho_all = round((dat_tho_all / tong_tho_all * 100), 2) if tong_tho_all > 0 else 0
    tyle_sau_all = round((dat_sau_all / tong_tho_all * 100), 2) if tong_tho_all > 0 else 0  # HNI: dùng tong_tho_all
    
    unit_stats.append({
        'Đơn vị': 'TTVT Sơn Tây',
        'Tổng phiếu (Thô)': int(tong_tho_all),
        'Phiếu GT (KĐ→Đ)': int(so_phieu_gt_all),
        'Tổng phiếu (Sau GT)': int(tong_sau_all),  # = Tổng thô
...
def create_c11_comparison_report(exclusion_ids, output_dir):
    """
    Tạo báo cáo so sánh C1.1 (SM4-C11) trước/sau giảm trừ theo cơ chế HNI
    ...
    try:
        print("\n" + "="*80)
        print("TẠO BÁO CÁO SO SÁNH C1.1 (SM4-C11) TRƯỚC/SAU GIẢM TRỪ - THEO CƠ CHẾ HNI")
        print("="*80)
        ...
        # Đếm số phiếu trong danh sách GT mà KHÔNG ĐẠT
        df_gt = df_raw[df_raw['BAOHONG_ID_STR'].isin(exclusion_ids)]
        num_gt_khong_dat = len(df_gt[df_gt['DAT_TT_KO_HEN'] != 1])
        print(f"✅ Số phiếu GT (không đạt → đạt): {num_gt_khong_dat}")
        
        # Tính thống kê TRƯỚC giảm trừ
        print("\n✓ Đang tính thống kê TRƯỚC giảm trừ...")
        df_stats_before = calculate_statistics(df_raw, has_ten_doi)
        
        # Tính thống kê SAU giảm trừ - THEO CƠ CHẾ HNI
        print("✓ Đang tính thống kê SAU giảm trừ (HNI: chuyển trạng thái, giữ mẫu số)...")
        df_stats_after = calculate_statistics_hni(df_raw, exclusion_ids, has_ten_doi)
        ...
        # Sắp xếp cột theo thứ tự mong muốn (HNI: Tổng phiếu không đổi)
        if has_ten_doi:
            column_order = [
                'TEN_DOI', 'NVKT',
                'Tổng phiếu (Thô)', 'Tổng phiếu (Sau GT)',  # Sẽ bằng nhau
                'Số phiếu đạt (Thô)', 'Số phiếu đạt (Sau GT)',
                'Số phiếu GT',  # Số phiếu được chuyển từ KĐ→Đ
        ...
        # Tạo sheet tổng hợp - HNI: Tổng phiếu sau GT = Tổng phiếu thô
        tong_before = df_stats_before['Tổng phiếu'].sum()
        tong_after = tong_before  # HNI: giữ nguyên mẫu số
        ...
        tyle_after = round((dat_after / tong_before * 100), 2) if tong_before > 0 else 0  # HNI: dùng tong_before
        
        df_tongke = pd.DataFrame([{
            'Chỉ tiêu': 'C1.1 - Tỷ lệ phiếu sửa chữa BRCD đúng quy định (không tính hẹn) - HNI',
            'Tổng phiếu (Thô)': tong_before,
            'Phiếu GT (KĐ→Đ)': num_gt_khong_dat,  # Số phiếu chuyển trạng thái
            'Tổng phiếu (Sau GT)': tong_after,  # = Tổng thô
        ...
        # Lấy danh sách phiếu được giảm trừ với chi tiết
        ...
        # Tính thống kê theo đơn vị (Tổ) - HNI
        df_unit_stats = calculate_unit_stats_hni(df_stats_before, df_stats_after)
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            df_loai_tru.to_excel(writer, sheet_name='DS_phieu_giam_tru', index=False)
        
        print(f"✅ Đã tạo báo cáo so sánh C1.1 (SM4-C11) - HNI")
        print(f"   - Tổng phiếu thô: {tong_before}")
        print(f"   - Phiếu GT (không đạt→đạt): {num_gt_khong_dat}")
        print(f"   - Tổng phiếu sau GT: {tong_after} (= Tổng thô)")
        ...
        return {
            ...
            'loai_tru': num_gt_khong_dat,  # Số phiếu chuyển trạng thái
...
def create_c11_sm2_comparison_report(exclusion_ids, output_dir):
    """
    Tạo báo cáo so sánh C1.1 SM2 trước/sau giảm trừ theo cơ chế HNI
    ...
    try:
        print("\n" + "="*80)
        print("TẠO BÁO CÁO SO SÁNH C1.1 SM2 TRƯỚC/SAU GIẢM TRỪ - THEO CƠ CHẾ HNI")
        print("="*80)
        ...
        # Đếm số phiếu trong danh sách GT mà KHÔNG ĐẠT
        df_gt = df_raw[df_raw['BAOHONG_ID_STR'].isin(exclusion_ids)]
        num_gt_khong_dat = len(df_gt[df_gt['PHIEU_DAT'] != 1])
        print(f"✅ Số phiếu GT (không đạt → đạt): {num_gt_khong_dat}")
        ...
        # Tính thống kê SAU giảm trừ - THEO CƠ CHẾ HNI
        print("✓ Đang tính thống kê SAU giảm trừ (HNI: chuyển trạng thái, giữ mẫu số)...")
        df_stats_after = calculate_statistics_hni(df_raw, exclusion_ids, has_ten_doi, dat_column='PHIEU_DAT', dat_value=1)
        ...
        df_tongke = pd.DataFrame([{
            'Chỉ tiêu': 'C1.1 SM2 - Tỷ lệ phiếu sửa chữa BRCD trong 72h - HNI',
            'Tổng phiếu (Thô)': tong_before,
            'Phiếu GT (KĐ→Đ)': num_gt_khong_dat,
            'Tổng phiếu (Sau GT)': tong_after,  # = Tổng thô
            'Phiếu đạt (Thô)': dat_before,
            'Phiếu đạt (Sau GT)': dat_after,
            'Tỷ lệ % (Thô)': tyle_before,
        ...
        # Lấy danh sách phiếu được giảm trừ
        df_loai_tru = df_raw[df_raw['BAOHONG_ID_STR'].isin(exclusion_ids)].copy()
        ...
        print(f"✅ Đã tạo báo cáo so sánh C1.1 SM2 - HNI")
        print(f"   - Tổng phiếu thô: {tong_before}")
        print(f"   - Phiếu GT (không đạt→đạt): {num_gt_khong_dat}")
        print(f"   - Tổng phiếu sau GT: {tong_after} (= Tổng thô)")
        print(f"   - Tỷ lệ thô: {tyle_before}% -> Sau GT: {tyle_after}%")
...
def create_c12_comparison_report(exclusion_ids, output_dir):
    """
    Tạo báo cáo so sánh C1.2 trước/sau giảm trừ theo cơ chế HNI
    Chỉ tiêu: Tỷ lệ thuê bao báo hỏng dịch vụ BRCĐ lặp lại
    
    CƠ CHẾ HNI cho C1.2 (càng thấp càng tốt):
    - Mẫu số (Báo hỏng): Giữ nguyên = Tổng phiếu báo hỏng thô
    - Tử số (HLL): = Số phiếu HLL gốc - Số phiếu HLL được giảm trừ
    
    Công thức: Tỷ lệ HLL = (Số phiếu HLL đã giảm) / (Số phiếu báo hỏng thô) * 100
    ...
    try:
        print("\n" + "="*80)
        print("TẠO BÁO CÁO SO SÁNH C1.2 (SM1-C12 + SM2-C12) - THEO CƠ CHẾ HNI")
        print("="*80)
        print("📌 HNI C1.2: Giữ nguyên mẫu số (báo hỏng), giảm tử số (HLL) nếu được GT")
        ...
        # ========== XỬ LÝ DỮ LIỆU SAU GIẢM TRỪ - HNI ==========
        print("\n" + "-"*40)
        print("XỬ LÝ DỮ LIỆU SAU GIẢM TRỪ (HNI)")
        print("-"*40)
        ...
        # Đếm số phiếu HLL bị giảm trừ
        df_sm1_gt = df_sm1_raw[df_sm1_raw['BAOHONG_ID_STR'].isin(exclusion_ids)]
        num_hll_gt = len(df_sm1_gt)
        print(f"✅ Số phiếu HLL được giảm trừ: {num_hll_gt}")
        
        # HNI: Tính số phiếu HLL sau GT = HLL gốc - HLL giảm trừ (không loại bỏ mẫu số)
        def calculate_hll_by_nvkt_hni(df_raw, exclusion_ids, has_ten_doi):
            """Tính HLL theo HNI: giảm số HLL nếu phiếu đó được GT (đếm MA_TB duy nhất)"""
            report_data = []
            if has_ten_doi:
                for (ten_doi, nvkt), group_df in df_raw.groupby(['TEN_DOI', 'NVKT']):
                    if pd.isna(nvkt):
                        continue
                    # Số phiếu HLL gốc (đếm MA_TB duy nhất)
                    so_phieu_hll_goc = group_df['MA_TB'].nunique()
                    # Số phiếu trong nhóm này bị GT (đếm MA_TB duy nhất)
                    df_gt = group_df[group_df['BAOHONG_ID_STR'].isin(exclusion_ids)]
                    num_gt_in_group = df_gt['MA_TB'].nunique()
                    # HLL sau GT = max(0, HLL gốc - số phiếu GT)
                    so_phieu_hll_sau = max(0, so_phieu_hll_goc - num_gt_in_group)
                    report_data.append({
                        'TEN_DOI': ten_doi,
                        'NVKT': nvkt,
                        'Số phiếu HLL': so_phieu_hll_sau,
                        'Số phiếu GT': num_gt_in_group
                    })
            else:
                for nvkt, group_df in df_raw.groupby('NVKT'):
                    if pd.isna(nvkt):
                        continue
                    so_phieu_hll_goc = group_df['MA_TB'].nunique()
                    df_gt = group_df[group_df['BAOHONG_ID_STR'].isin(exclusion_ids)]
        ...
        df_hll_after = calculate_hll_by_nvkt_hni(df_sm1_raw, exclusion_ids, has_ten_doi)
        print(f"✅ HLL sau GT (HNI): {df_hll_after['Số phiếu HLL'].sum()} phiếu")
        
        # ========== ĐỌC SM2-C12 (Tổng phiếu báo hỏng) ==========
        print("\n" + "-"*40)
        print("XỬ LÝ SM2-C12 (Tổng phiếu báo hỏng) - HNI: GIỮ NGUYÊN")
        print("-"*40)
        ...
        # HNI: Giữ nguyên số phiếu báo hỏng (mẫu số không đổi - đếm MA_TB duy nhất)
        ...
        # HNI: Báo hỏng sau GT = Báo hỏng thô (giữ nguyên mẫu số)
        df_bh_after = df_bh_before.copy()  # Giữ nguyên
        
        print(f"✅ Báo hỏng (HNI - giữ nguyên): {df_bh_before['Số phiếu báo hỏng'].sum()} phiếu")
        
        # ========== KẾT HỢP VÀ TÍNH TỶ LỆ ==========
        print("\n" + "-"*40)
        print("TẠO BÁO CÁO SO SÁNH - HNI")
        print("-"*40)
        ...
        # Tính tỷ lệ cho dữ liệu TRƯỚC giảm trừ
        ...
        # Merge HLL với báo hỏng (SAU giảm trừ HNI)
        ...
        # HNI: Tỷ lệ = HLL sau GT / Báo hỏng thô
        ...
        df_comparison = pd.merge(
            ...
            df_after[merge_cols + ['Số phiếu HLL', 'Số phiếu báo hỏng', 'Số phiếu GT', 'Tỷ lệ HLL %']],
            on=merge_cols,
            how='outer',
            suffixes=(' (Thô)', ' (Sau GT)')
        )
        ...
        if has_ten_doi:
            column_order = [
                'TEN_DOI', 'NVKT',
                'Số phiếu HLL (Thô)', 'Số phiếu HLL (Sau GT)', 'Số phiếu GT',
        ...
        # Tạo sheet tổng hợp - HNI
        hll_tho = df_before['Số phiếu HLL'].sum()
        hll_sau = df_after['Số phiếu HLL'].sum()
        bh_tho = df_before['Số phiếu báo hỏng'].sum()
        bh_sau = bh_tho  # HNI: Giữ nguyên mẫu số
        num_hll_giam = hll_tho - hll_sau
        tyle_tho = round((hll_tho / bh_tho * 100), 2) if bh_tho > 0 else 0
        tyle_sau = round((hll_sau / bh_tho * 100), 2) if bh_tho > 0 else 0  # HNI: dùng bh_tho
        
        df_tongke = pd.DataFrame([{
            'Chỉ tiêu': 'C1.2 - Tỷ lệ thuê bao báo hỏng BRCĐ lặp lại - HNI',
            'Phiếu HLL (Thô)': hll_tho,
            'Phiếu HLL giảm': num_hll_giam,
            'Phiếu HLL (Sau GT)': hll_sau,
            'Phiếu báo hỏng (Thô)': bh_tho,
            'Phiếu báo hỏng (Sau GT)': bh_sau,  # = Thô
        ...
        merge_cols = ['TEN_DOI', 'NVKT'] if 'TEN_DOI' in df_before.columns else ['NVKT']
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            df_unit_stats.to_excel(writer, sheet_name='Thong_ke_theo_don_vi', index=False)
            df_tongke.to_excel(writer, sheet_name='Thong_ke_tong_hop', index=False)
            df_loai_tru_sm1.to_excel(writer, sheet_name='DS_phieu_giam_tru', index=False)
        
        print(f"\n✅ Đã tạo báo cáo so sánh C1.2 - HNI")
        print(f"   - Phiếu HLL: {hll_tho} -> {hll_sau} (giảm: {num_hll_giam})")
        print(f"   - Phiếu báo hỏng: {bh_tho} (HNI: giữ nguyên)")
...
def create_c14_comparison_report(exclusion_ids, output_dir):
    """
    Tạo báo cáo so sánh C1.4 - Độ hài lòng khách hàng theo cơ chế HNI
    
    CƠ CHẾ HNI cho C1.4:
    - Mẫu số (Tổng phiếu KS): Giữ nguyên = Tổng phiếu KS thô
    - Tử số (Phiếu hài lòng): = (Tổng thô - KHL thô) + Phiếu KHL được GT
    
    Công thức: Tỷ lệ HL = (Tổng KS thô - KHL sau GT) / Tổng KS thô * 100
    ...
    try:
        print("\n" + "="*80)
        print("TẠO BÁO CÁO SO SÁNH C1.4 - ĐỘ HÀI LÒNG KHÁCH HÀNG - THEO CƠ CHẾ HNI")
        print("="*80)
        print("📌 HNI C1.4: Giữ nguyên mẫu số (Tổng KS), giảm tử số (KHL) nếu được GT")
        ...
        # Đếm số phiếu KHL được giảm trừ
        df_gt = df_ks[df_ks['BAOHONG_ID_STR'].isin(exclusion_ids)]
        num_khl_gt = len(df_gt[df_gt['IS_KHL'] == 1])  # Chỉ đếm phiếu KHL trong danh sách GT
        print(f"✅ Số phiếu KHL được giảm trừ: {num_khl_gt}")
        ...
        # ========== TÍNH TOÁN SAU GIẢM TRỪ - HNI ==========
        # HNI: Giữ nguyên Tổng KS, giảm số KHL nếu phiếu đó được GT
        def calculate_c14_hni(df_ks, exclusion_ids, has_ten_doi):
            """Tính C1.4 theo HNI: giảm KHL nếu phiếu đó được GT"""
            report_data = []
            group_cols = ['TEN_DOI', 'NVKT'] if has_ten_doi else ['NVKT']
            
            for group_key, group_df in df_ks.groupby(group_cols):
                # Tổng phiếu KS = Tổng thô (giữ nguyên mẫu số)
                tong_ks = len(group_df)
                
                # Số phiếu KHL gốc
                so_khl_goc = group_df['IS_KHL'].sum()
                
                # Số phiếu KHL trong nhóm này được GT
                df_gt_group = group_df[group_df['BAOHONG_ID_STR'].isin(exclusion_ids)]
                so_khl_gt = df_gt_group['IS_KHL'].sum()
                
                # KHL sau GT = KHL gốc - KHL được GT
                so_khl_sau = max(0, so_khl_goc - so_khl_gt)
                
                # Tỷ lệ HL với mẫu số = Tổng KS thô
                ty_le_hl = round((tong_ks - so_khl_sau) / tong_ks * 100, 2) if tong_ks > 0 else 100
                ...
                if has_ten_doi:
                    ...
                    report_data.append({
                        'TEN_DOI': ten_doi,
                        'NVKT': nvkt,
                        'Tổng phiếu KS': tong_ks,  # Giữ nguyên = thô
                        'Số phiếu KHL': so_khl_sau,  # KHL gốc - KHL GT
                        'Số phiếu GT': int(so_khl_gt),
                        'Tỷ lệ HL (%)': ty_le_hl
                    })
                else:
                    report_data.append({
                        'NVKT': group_key,
                        'Tổng phiếu KS': tong_ks,
                        'Số phiếu KHL': so_khl_sau,
                        'Số phiếu GT': int(so_khl_gt),
                        'Tỷ lệ HL (%)': ty_le_hl
                    })
        ...
        df_after = calculate_c14_hni(df_ks, exclusion_ids, has_ten_doi)
        ...
        if has_ten_doi:
            column_order = [
                ...
                'Tổng phiếu KS (Sau GT)', 'Số phiếu KHL (Sau GT)', 'Số phiếu GT', 'Tỷ lệ HL (%) (Sau GT)',
                'Chênh lệch %'
            ]
            df_comparison = df_comparison.sort_values(['TEN_DOI', 'NVKT']).reset_index(drop=True)
        else:
        ...
        # HNI: Tổng KS sau = Tổng KS thô (giữ nguyên mẫu số)
        tong_ks_sau = tong_ks_tho
        tong_khl_sau = df_after['Số phiếu KHL'].sum()
        tyle_sau = round((tong_ks_tho - tong_khl_sau) / tong_ks_tho * 100, 2) if tong_ks_tho > 0 else 100  # HNI: dùng tong_ks_tho
        
        df_tongke = pd.DataFrame([{
            'Chỉ tiêu': 'C1.4 - Độ hài lòng khách hàng - HNI',
            'Tổng phiếu KS (Thô)': tong_ks_tho,
            'Tổng phiếu KHL (Thô)': tong_khl_tho,
            'Phiếu KHL giảm': num_khl_gt,
            'Tổng phiếu KS (Sau GT)': tong_ks_sau,  # = Thô
        ...
        print(f"\n✅ Đã tạo báo cáo so sánh C1.4 - HNI")
        print(f"   - Tổng phiếu KS: {tong_ks_tho} (HNI: giữ nguyên)")
        print(f"   - Phiếu KHL: {tong_khl_tho} -> {tong_khl_sau} (giảm: {num_khl_gt})")
...
def create_c15_comparison_report(exclusion_ids, output_dir):
    """
    Tạo báo cáo so sánh C1.5 - Tỷ lệ thiết lập dịch vụ BRCĐ đạt thời gian quy định theo cơ chế HNI
    
    CƠ CHẾ HNI cho C1.5:
    - Mẫu số (Tổng Hoàn công): Giữ nguyên = Tổng Hoàn công thô
    - Tử số (Phiếu đạt): = Phiếu đạt gốc + Phiếu không đạt được GT (chuyển thành đạt)
    
    Công thức: Tỷ lệ đạt = (Phiếu đạt gốc + Phiếu GT không đạt) / Tổng Hoàn công thô * 100
    
    Args:
        exclusion_ids: Set các HDTB_ID cần giảm trừ
        output_dir: Thư mục xuất kết quả
    ...
    try:
        print("\n" + "="*80)
        print("TẠO BÁO CÁO SO SÁNH C1.5 - TỶ LỆ THIẾT LẬP DỊCH VỤ BRCĐ - THEO CƠ CHẾ HNI")
        print("="*80)
        print("📌 HNI C1.5: Giữ nguyên mẫu số (Tổng HC), chuyển phiếu không đạt → đạt nếu được GT")
        ...
        # Đếm số phiếu không đạt được giảm trừ (sẽ chuyển thành đạt)
        df_gt = df_hc[df_hc['HDTB_ID_STR'].isin(exclusion_ids)]
        num_gt_khong_dat = len(df_gt[df_gt['IS_DAT'] == 0])  # Chỉ đếm phiếu không đạt trong danh sách GT
        print(f"✅ Số phiếu không đạt được GT (→ đạt): {num_gt_khong_dat}")
        ...
        # ========== TÍNH TOÁN SAU GIẢM TRỪ - HNI ==========
        def calculate_c15_hni(df_hc, exclusion_ids, has_ten_doi):
            """Tính C1.5 theo HNI: chuyển phiếu không đạt → đạt nếu được GT"""
            report_data = []
            group_cols = ['TEN_DOI', 'NVKT'] if has_ten_doi else ['NVKT']
            
            for group_key, group_df in df_hc.groupby(group_cols):
                # Tổng Hoàn công = Tổng thô (giữ nguyên mẫu số)
                tong_hc = len(group_df)
                
                # Số phiếu đạt gốc
                so_dat_goc = group_df['IS_DAT'].sum()
                
                # Số phiếu không đạt trong nhóm này được GT (sẽ chuyển thành đạt)
                df_gt_group = group_df[group_df['HDTB_ID_STR'].isin(exclusion_ids)]
                so_khong_dat_gt = len(df_gt_group[df_gt_group['IS_DAT'] == 0])
                
                # Phiếu đạt sau GT = Đạt gốc + Không đạt được GT
                so_dat_sau = so_dat_goc + so_khong_dat_gt
                so_khong_dat_sau = tong_hc - so_dat_sau
                
                # Tỷ lệ đạt với mẫu số = Tổng HC thô
                ty_le_dat = round(so_dat_sau / tong_hc * 100, 2) if tong_hc > 0 else 100
                ...
                if has_ten_doi:
                    ...
                    report_data.append({
                        'TEN_DOI': ten_doi,
                        'NVKT': nvkt,
                        'Tổng Hoàn công': tong_hc,  # Giữ nguyên = thô
                        'Phiếu đạt': so_dat_sau,
                        'Phiếu không đạt': so_khong_dat_sau,
                        'Số phiếu GT': int(so_khong_dat_gt),
                        'Tỷ lệ đạt (%)': ty_le_dat
                    })
                else:
                    report_data.append({
        ...
        df_after = calculate_c15_hni(df_hc, exclusion_ids, has_ten_doi)
        ...
        if has_ten_doi:
            column_order = [
                ...
                'Tổng Hoàn công (Sau GT)', 'Phiếu đạt (Sau GT)', 'Phiếu không đạt (Sau GT)', 'Số phiếu GT', 'Tỷ lệ đạt (%) (Sau GT)',
                'Chênh lệch %'
            ]
            df_comparison = df_comparison.sort_values(['TEN_DOI', 'NVKT']).reset_index(drop=True)
        else:
        ...
        # HNI: Tổng HC sau = Tổng HC thô (giữ nguyên mẫu số)
        tong_hc_sau = tong_hc_tho
        tong_dat_sau = df_after['Phiếu đạt'].sum()
        tyle_sau = round(tong_dat_sau / tong_hc_tho * 100, 2) if tong_hc_tho > 0 else 100  # HNI: dùng tong_hc_tho
        
        df_tongke = pd.DataFrame([{
            'Chỉ tiêu': 'C1.5 - Tỷ lệ thiết lập dịch vụ BRCĐ đạt TG quy định - HNI',
            'Tổng Hoàn công (Thô)': tong_hc_tho,
            'Phiếu đạt (Thô)': tong_dat_tho,
            'Phiếu GT (KĐ→Đ)': num_gt_khong_dat,
            'Tổng Hoàn công (Sau GT)': tong_hc_sau,  # = Thô
        ...
        print(f"\n✅ Đã tạo báo cáo so sánh C1.5 - HNI")
        print(f"   - Tổng Hoàn công: {tong_hc_tho} (HNI: giữ nguyên)")
        print(f"   - Phiếu đạt: {tong_dat_tho} -> {tong_dat_sau} (+{num_gt_khong_dat} phiếu GT)")
    ...
    try:
        ...
        def read_unit_scores_from_comparison_files(folder):
            ...
            if 'c14_tho' in df_all.columns:
                ...
                print("  ⚠️ Không có dữ liệu C1.4, dùng giá trị mặc định = 5")
                df_all['Diem_C1.4 (Trước)'] = 5
                df_all['Diem_C1.4 (Sau)'] = 5
                df_all['Diem_C1.4 (CL)'] = 0
            ...
            if 'c15_tho' in df_all.columns:
                ...
                print("  ⚠️ Không có dữ liệu C1.5, dùng giá trị mặc định = 5")
                df_all['Diem_C1.5 (Trước)'] = 5
                df_all['Diem_C1.5 (Sau)'] = 5
                df_all['Diem_C1.5 (CL)'] = 0
...
def process_exclusion_reports():
    """
    Hàm chính: Tạo tất cả báo cáo so sánh giảm trừ theo cơ chế HNI
    ...
    Quy trình:
    1. Đọc danh sách BAOHONG_ID cần giảm trừ
    2. Tạo thư mục xuất kết quả (kq_sau_giam_tru_hni)
    3. Tạo báo cáo so sánh cho từng chỉ tiêu
    4. Tạo báo cáo tổng hợp
    """
    try:
        print("\n" + "="*80)
        print("BẮT ĐẦU TẠO BÁO CÁO SO SÁNH GIẢM TRỪ - THEO CƠ CHẾ HNI")
        print("="*80)
        print("📌 CƠ CHẾ HNI: Giữ nguyên mẫu số, chỉ chuyển trạng thái phiếu từ KĐ→Đ")
        
        # 1. Đọc danh sách giảm trừ
        exclusion_ids = load_exclusion_list()
        
        if not exclusion_ids:
            print("⚠️ Không có phiếu nào cần giảm trừ. Bỏ qua việc tạo báo cáo.")
            return False
        
        # 2. Tạo thư mục xuất kết quả - THAY ĐỔI: kq_sau_giam_tru_hni
        output_dir = os.path.join("downloads", "kq_sau_giam_tru_hni")
        ...
        # C1.1 SM2 - Sử dụng file giảm trừ riêng
        print("\n" + "-"*80)
        print("Đọc danh sách giảm trừ riêng cho C1.1 SM2 (TP1)")
        print("-"*80)
        exclusion_ids_c11_sm2 = load_exclusion_list("du_lieu_tham_chieu/LOAI_TRU_C1.1_TP1.xlsx")
        
        if exclusion_ids_c11_sm2:
            print(f"✅ Sử dụng {len(exclusion_ids_c11_sm2)} phiếu giảm trừ từ LOAI_TRU_C1.1_TP1.xlsx")
            result = create_c11_sm2_comparison_report(exclusion_ids_c11_sm2, output_dir)
        else:
            print(f"⚠️ Không tìm thấy file LOAI_TRU_C1.1_TP1.xlsx, sử dụng danh sách giảm trừ chung")
        ...
        if results:
            print("\n" + "="*80)
            print("TẠO BÁO CÁO TỔNG HỢP TẤT CẢ CHỈ TIÊU - HNI")
            print("="*80)
            ...
            df_summary.columns = [
                'Chỉ tiêu', 
                'Tổng phiếu (Thô)', 
                'Phiếu GT (KĐ→Đ)',  # Thay đổi tên cột
                'Tổng phiếu (Sau GT)',
                'Tỷ lệ % (Thô)',
                'Tỷ lệ % (Sau GT)'
            ]
            ...
            summary_file = os.path.join(output_dir, "Tong_hop_giam_tru_HNI.xlsx")
            df_summary.to_excel(summary_file, index=False)
            
            print(f"✅ Đã tạo báo cáo tổng hợp: {summary_file}")
            print("\n" + "-"*80)
            print("TỔNG KẾT (HNI - Giữ mẫu số, chuyển trạng thái):")
            print("-"*80)
        ...
        print("\n" + "="*80)
        print("TÍNH TOÁN KPI CÁ NHÂN SAU GIẢM TRỪ CHO NVKT - HNI")
        print("="*80)
        ...
        df_kpi_nvkt = kpi_calculator_v2.tinh_diem_kpi_nvkt_sau_giam_tru(
        ...
        if df_kpi_nvkt is not None:
            # 6. [V2] Bỏ tạo Word report tại đây - sẽ gọi từ report_generator_v2 riêng
        ...
        print("\n" + "="*80)
        print("✅ HOÀN THÀNH TẤT CẢ CÁC BƯỚC XỬ LÝ GIẢM TRỪ - HNI")
