...
async function initI15Page() {
    await initSHCDetailDownloader('k1');
    await loadI15Data();
    await loadSHCVariationData();
}
...
function bindDateFilter() {
    const input = document.getElementById('i15-date-input');
    const applyButton = document.getElementById('i15-date-apply');
...
function syncI15DateState(data) {
    const input = document.getElementById('i15-date-input');
    if (input) {
        input.value = data.selected_date || getRequestedDate();
    }

    const meta = document.getElementById('i15-date-meta');
    if (!meta) {
        return;
    }
    ...
    try {
        ...
        if (data) {
            ...
            if (data.tong_hop) {
                ...
                document.getElementById('i15-table-container').innerHTML = summaryHtml;
            }

            // Bảng 2-5: Chi tiết SHC theo Đơn vị
            if (data.don_vi && Object.keys(data.don_vi).length > 0) {
                const donViNames = Object.keys(data.don_vi);
                renderI15DetailTabs(donViNames);
                renderI15DetailTable(data.don_vi[donViNames[0]], donViNames[0], fileInfo);

                // Store data for tab switching
                window.i15DetailData = data;
                window.i15FileInfo = fileInfo;
            }
            ...
            if (data.shc_theo_sa) {
                ...
                document.getElementById('i15-sa-table-container').innerHTML = saHtml;
            }
        }
    } catch (error) {
        console.error('Error loading I1.5 data:', error);
        showError('Không thể tải dữ liệu I1.5: ' + error.message, 'i15-table-container');
    }
}

/**
 * Render I1.5 detail tabs
 */
function renderI15DetailTabs(sheetNames) {
    const tabsContainer = document.getElementById('i15-don-vi-tabs');
    ...
    tabsContainer.querySelectorAll('.excel-tab').forEach(tab => {
        tab.addEventListener('click', function () {
            ...
            renderI15DetailTable(window.i15DetailData.don_vi[donViName], donViName, window.i15FileInfo);
        });
    });
}

/**
 * Render I1.5 detail table
 */
function renderI15DetailTable(sheetData, sheetName, fileInfo = null) {
    ...
    document.getElementById('i15-don-vi-tables-container').innerHTML = html;
}

/**
 * Download I1.5 Excel file
 */
function downloadExcelI15() {
    const requestedDate = getRequestedDate();
    const query = requestedDate ? `?date=${encodeURIComponent(requestedDate)}` : '';
    window.location.href = `/download/excel-i15${query}`;
}

/**
 * Load SHC Variation data (T-1 comparison)
 */
async function loadSHCVariationData() {
    try {
        const endpoint = getRequestedDate()
            ? `/api/shc-variation-data?date=${encodeURIComponent(getRequestedDate())}`
            : '/api/shc-variation-data';
        const data = await API.fetchData(endpoint);
        ...
        if (data) {
            ...
            if (data.theo_don_vi) {
                ...
                document.getElementById('shc-variation-don-vi-container').innerHTML = donViHtml;
            }
            ...
            if (data.chi_tiet_nvkt && Object.keys(data.chi_tiet_nvkt).length > 0) {
                ...
                renderSHCNVKTTabs(donViNames);
                renderSHCNVKTTable(data.chi_tiet_nvkt[donViNames[0]], donViNames[0], fileInfo);

                // Store data for tab switching
                window.shcNvktData = data.chi_tiet_nvkt;
                window.shcNvktFileInfo = fileInfo;
            }
        }
    } catch (error) {
        console.error('Error loading SHC Variation data:', error);
        // Show error in containers if they exist
        const donViContainer = document.getElementById('shc-variation-don-vi-container');
        const nvktContainer = document.getElementById('shc-variation-nvkt-container');
        if (donViContainer) {
...
/**
 * Render SHC NVKT tabs
 */
function renderSHCNVKTTabs(donViNames) {
    const tabsContainer = document.getElementById('shc-nvkt-tabs');
    if (!tabsContainer) return;
    ...
    tabsContainer.querySelectorAll('.excel-tab').forEach(tab => {
        tab.addEventListener('click', function () {
            ...
            renderSHCNVKTTable(window.shcNvktData[donViName], donViName, window.shcNvktFileInfo);
        });
    });
}

/**
 * Render SHC NVKT table for a specific Đơn vị
 */
function renderSHCNVKTTable(sheetData, donViName, fileInfo = null) {
    ...
    document.getElementById('shc-variation-nvkt-container').innerHTML = html;
}

async function initSHCDetailDownloader(defaultReportType = 'k1') {
