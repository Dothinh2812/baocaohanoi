/* ========================================
   TON KHO VAT TU QUANG OAI PAGE JAVASCRIPT
   ======================================== */

document.addEventListener('DOMContentLoaded', function () {
    loadTonKhoVatTuQOI();
    loadTonThuongDungQOI();
});

async function loadTonThuongDungQOI() {
    var container = document.getElementById('ton-thuong-dung-qoi-container');
    if (!container) return;

    try {
        var response = await fetch('/api/ton-kho-vat-tu-qoi-tot-thuong-dung');
        if (!response.ok) {
        ...
        if (data.columns && data.data) {
            container.innerHTML = createExcelTable(data, 'Tổng hợp tồn vật tư thường dùng (tốt) - Quảng Oai', {
        ...
        console.error('Error loading ton thuong dung QOI:', error);
...
async function loadTonKhoVatTuQOI() {
    var container1 = document.getElementById('ton-kho-nvkt-container');
    var container2 = document.getElementById('ton-theo-loai-qoi-container');

    try {
        var response = await fetch('/api/ton-kho-vat-tu-qoi');
        if (!response.ok) {
            throw new Error('HTTP error ' + response.status);
        }
        ...
        if (data.sheets) {
            ...
            if (container1) {
                ...
                if (sheetData1) {
                    container1.innerHTML = createExcelTable(sheetData1, 'Tồn kho Quảng Oai theo NVKT', {
                        showRowNumbers: false,
                        maxHeight: 'none',
                        fileInfo: data.file_info,
                        enableFilter: true
        ...
        console.error('Error loading ton kho vat tu QOI:', error);
