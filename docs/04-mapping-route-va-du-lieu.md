...
| `/dao-tao-sat-hach` | `/api/training/*`, gồm `POST /api/training/exams/<id>/cancel`, `/download/training/*` | `n/a` | SQLite ghi được per-instance `DASHV4_TRAINING_DB_PATH` (mặc định `runtime_app/<unit>/training.db`), migration 1-11 | `n/a` | `compatible` | Module độc lập, không đọc/ghi `report_history.db` và không áp dụng date-contract. Route/API kiểm tra RBAC module theo scope server-side. Backend đã có AI queue lease/recovery, fixed template, close/cancel/finalize, attempt/report snapshot checksum; chưa xác nhận hoàn tất UI, production OpenAI hoặc thi lại. |
