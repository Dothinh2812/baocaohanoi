...
| `/dao-tao-sat-hach` | `/api/training/*`, gồm `GET /api/training/questions`, `GET /api/training/questions/<version>` (Cache-Control: no-store), `POST /api/training/questions/{validate,import,approve,reject,publish}`, `GET /api/training/templates`, `GET /api/training/templates/<id>`, `POST /api/training/templates`, `GET /api/training/exams`, `GET /api/training/exams/<id>`, `GET /api/training/exams/<id>/assignments`, `GET /api/training/users`, `POST /api/training/exams`, `POST /api/training/exams/<id>/assignments`, `POST /api/training/exams/<id>/cancel`, `POST /api/training/exams/<id>/ready`, `POST /api/training/exams/<id>/open`, `GET /api/training/my-assignments`, `POST /api/training/exams/<id>/finalize`, `GET /api/training/exams/<id>/report`, `/download/training/*` | `n/a` | SQLite ghi được per-instance `DASHV4_TRAINING_DB_PATH` (mặc định `runtime_app/<unit>/training.db`), migration 1-11 | `n/a` | `compatible` | Module độc lập, không đọc/ghi `report_history.db` và không áp dụng date-contract. UI-2 đã hoàn tất: panel ngân hàng câu hỏi cho editor/exam-manager, list phân trang/filter, detail DTO quản trị (gồm scoring metadata/lịch sử), validate/import draft, duyệt/từ chối/phát hành có xác nhận và state transition hợp lệ. UI-3 đã hoàn tất: template panel (list/create/detail với question picker), exam panel (list/create/detail/lifecycle/assignment management), read model APIs cho templates/exams/users. CAS cho review/publish transitions. Classification domain suy ra từ evidence -> knowledge block, không thêm migration. UI-4 đã hoàn tất: learner experience với `GET /api/training/my-assignments`, frontend assignment catalog (`training-my-exams.js`) và attempt workspace (`training-attempt.js`) với autosave CAS, countdown, submit confirm. UI-5 đã hoàn tất: report dashboard frontend (`training-reports.js`) hiển thị danh sách finalized exams, chi tiết báo cáo summary + individual, finalize action, export Excel. Route RBAC: learner blocked từ report/finalize/export. Snapshot bất biến, report không lộ đáp án/explanation/evidence. Chưa xác nhận production OpenAI hoặc thi lại. |
