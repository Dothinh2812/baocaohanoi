...
- UI-2 đã hoàn tất và có regression test trong `/dao-tao-sat-hach`: editor/exam-manager có panel ngân hàng câu hỏi với filter/phân trang, management detail DTO gồm scoring metadata và lịch sử, JSON validate/import chỉ tạo draft, và approve/reject/publish có xác nhận, RBAC/CSRF cùng state transition hợp lệ. Publish lặp lại hoặc review câu đã publish trả business conflict và không ghi thêm audit. Domain filter suy ra qua evidence -> `knowledge_blocks`; không có migration mới. Learner attempt route giữ `Cache-Control: no-store` và không trả đáp án, explanation, evidence, distractor rationales hoặc metadata chấm.
- UI-3 đã hoàn tất: template management (list/create/detail với question picker), exam management (list/create/detail/lifecycle/assignment), read model APIs (`GET /api/training/templates`, `GET /api/training/templates/<id>`, `GET /api/training/exams`, `GET /api/training/exams/<id>`, `GET /api/training/exams/<id>/assignments`, `GET /api/training/users`), JS behavioral test cho exam lifecycle, integration test cho tất cả routes. Hardening: `Cache-Control: no-store` trên management question detail, CAS cho review transitions và publish. Frontend: `static/js/training-templates.js`, `static/js/training-exams.js`.
- UI-4 đã hoàn tất: `GET /api/training/my-assignments` trả danh sách assignment learner, frontend `static/js/training-my-exams.js` hiển thị phân nhóm theo trạng thái (chưa bắt đầu/đang làm/đã hoàn thành/hết hạn), frontend `static/js/training-attempt.js` thực hiện làm bài với đồng hồ countdown từ `deadline_at_ms`, autosave debounce `client_revision` CAS, submit confirm + unanswered count, lock UI khi terminal/expires. JS behavioral test cho attempt workspace rendering. HTML panel learner trong `templates/pages/training/index.html`, CSS attempt workspace. Learner không thấy câu trả lời đúng, explanation, evidence hoặc metadata chấm qua API/HTML.
- Chưa ghi nhận hoàn tất báo cáo sau thi, kho tri thức, production OpenAI, hoặc luồng thi lại. Các phần này vẫn cần acceptance riêng trước khi coi là hoàn thành MVP.
