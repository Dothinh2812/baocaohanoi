...
class QuangChuDongCache:
    ...
    def get_active_payload(self):
        ...
        payload = dict(snapshot.get('active', {'alerts': [], 'excluded': [], 'sources': []}))
        payload['cache'] = dict(snapshot.get('cache_meta', self._fallback_cache_meta()))
        return payload

    def get_recovered_payload(self):
        snapshot = self._get_snapshot(wait_for_refresh_if_empty=True)
        return snapshot.get('recovered', [])

    def get_wide_area_payload(self):
        snapshot = self._get_snapshot(wait_for_refresh_if_empty=True)
        return snapshot.get('wide_area', [])

    def get_exclusion_list_payload(self):
        snapshot = self._get_snapshot(wait_for_refresh_if_empty=True)
        return snapshot.get('exclusion_list', [])

    def get_stats_payload(self):
        snapshot = self._get_snapshot(wait_for_refresh_if_empty=True)
        return snapshot.get('stats', {
            'active_by_doi_vt': {},
    ...
    def _set_snapshot(self, snapshot, snapshot_mtime):
        with self._state_lock:
